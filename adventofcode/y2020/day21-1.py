...
ALL_INGREDIENTS: Dict[str, int] = {}
INGREDIENT_ALLERGENS: Dict[str, str] = {}
FOUND_ALLERGENS: List[str] = []
FOUND_INGREDIENT: List[str] = []
        ...
        for num, allergen in enumerate(self.allergens):
            ...
            if not allergen in ALL_ALLERGENS:
                ...
                ALL_ALLERGENS[allergen] = [
                    x for x in ALL_ALLERGENS[allergen] if x in self.ingredients
                ]
...
while len(FOUND_ALLERGENS) < len(ALL_ALLERGENS):
    for allergen in ALL_ALLERGENS:
        if not allergen in FOUND_ALLERGENS:
            ALL_ALLERGENS[allergen] = [
                x for x in ALL_ALLERGENS[allergen] if x not in FOUND_INGREDIENT
            ]
            if len(ALL_ALLERGENS[allergen]) == 1:
                FOUND_ALLERGENS.append(allergen)
                FOUND_INGREDIENT.append(ALL_ALLERGENS[allergen][0])
...
for i in ALL_INGREDIENTS:
    ...
    for a in ALL_ALLERGENS:
        if i in ALL_ALLERGENS[a]:
            safe = False
            break
    if safe:
        count += ALL_INGREDIENTS[i]
