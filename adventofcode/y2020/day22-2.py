...
deck1: typing.List[int] = [int(x) for x in inFile[0][9:].strip().split("\n")]
deck2: typing.List[int] = [int(x) for x in inFile[1][9:].strip().split("\n")]
...
class CrabCombat:
    ...
    def play_round(self):
        ...
        if len(self.deck1) >= card1 and len(self.deck2) >= card2:
            ...
            roundWinner = self.run_sub_game(
                copy.deepcopy(newDeck1), copy.deepcopy(newDeck2)
            )
        else:
            if card1 > card2:
                roundWinner = 1
